...
class BaseIncludeFile_Access
{
	...
	public function delete()
	{
		...
		if ($this->id)
		{
			...
			$sql = "DELETE FROM ".constant("BASE_INCLUDE_FILE_TABLE")." WHERE id = :id";
			$res = $db->prepare($sql);
	...
	public function set_include_id($include_id)
	{
		...
		{
			$sql = "UPDATE ".constant("BASE_INCLUDE_FILE_TABLE")." SET include_id = :include_id WHERE id = :id";
			$res = $db->prepare($sql);
	...
	public function set_name($name)
	{
		...
		if ($this->id and $name)
		{
			$sql = "UPDATE ".constant("BASE_INCLUDE_FILE_TABLE")." SET name = :name WHERE id = :id";
			$res = $db->prepare($sql);
			$db->bind_value($res, ":id", $this->id, PDO::PARAM_INT);
	...
	public function set_checksum($checksum)
	{
		...
		{
			$sql = "UPDATE ".constant("BASE_INCLUDE_FILE_TABLE")." SET checksum = :checksum WHERE id = :id";
			$res = $db->prepare($sql);
			$db->bind_value($res, ":id", $this->id, PDO::PARAM_INT);
