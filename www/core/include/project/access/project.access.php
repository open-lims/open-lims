...
class Project_Access
{
	 ...
	 */
   	...
   	/**
   	 * @todo move this method to wrapper-class
   	 * @param string $name
   	 * @param array $template_id
   	 * @param array $organisation_unit_array
   	 * @return array
   	 */
   	public static function search_projects_without_subprojects($name, $template_array, $organisation_unit_array)
   	{
   		global $db;
   			...
   			(is_array($organisation_unit_array) and count($organisation_unit_array) >= 1))
   		{
			...
			foreach($organisation_unit_array as $key => $value)
			{
				...
				{
					$organisation_unit_sql .= " OR toid_organ_unit = '".$value."'";
				}
				else
				{
					$organisation_unit_sql .= "toid_organ_unit = '".$value."'";
				}
			}
			
			$add_sql .= $organisation_unit_sql.") AND toid_project IS NULL";
   	 ...
   	 * @return array
   	 */
   	public static function search_projects_with_subprojects($name, $template_array, $organisation_unit_array)
	{
   		global $db;
   			...
   			(is_array($organisation_unit_array) and count($organisation_unit_array) >= 1))
   		{
			...
			else
			{
   			...
   			if (is_array($template_array) and count($template_array) >= 1)
   			{
				...
				foreach($organisation_unit_array as $ou_key => $ou_value)
				{	
					$template_sql = "";
	   				...
	   				foreach($template_array as $template_key => $template_value)
	   				{
	   					if ($template_sql)
	   					{
	   						$template_sql .= " OR id IN (SELECT * FROM search_get_project_subprojects(".$ou_value.", NULL, ".$template_value."))";
	   					}
	   					else
	   					{
	   						$template_sql .= "id IN (SELECT * FROM search_get_project_subprojects(".$ou_value.", NULL, ".$template_value."))";
	   					}
	   				}
					
					if ($organisation_unit_sql)
					{
						$organisation_unit_sql .= " OR ".$template_sql;
					}
					else
					{
						$organisation_unit_sql .= $template_sql;
					}				
				}
   			...
   			else
   			{
				...
				foreach($organisation_unit_array as $key => $value)
				{
					if ($organisation_unit_sql)
					{
						$organisation_unit_sql .= " OR id IN (SELECT * FROM search_get_project_subprojects(".$value.", NULL, NULL))";
					}
					else
					{
						$organisation_unit_sql .= "id IN (SELECT * FROM search_get_project_subprojects(".$value.", NULL, NULL))";
					}
				}
				$add_sql .= $organisation_unit_sql.")";
   			}
