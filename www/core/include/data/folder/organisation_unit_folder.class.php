...
class OrganisationUnitFolder extends Folder implements ConcreteFolderCaseInterface, EventListenerInterface
{
	...
	public function create()
	{
		...
		{
			$organisation_unit = new OrganisationUnit($this->ci_organisation_unit_id);
			...
			$path->add_element($this->ci_organisation_unit_id);
			
			parent::ci_set_name($organisation_unit->get_name());
			parent::ci_set_toid($organisation_unit_folder_id);
			parent::ci_set_path($path->get_path_string());
			parent::ci_set_owner_id($organisation_unit->get_master_owner_id());
			if (($folder_id = parent::create()) != null)
			{
				$folder_is_organisation_unit_folder_access = new FolderIsOrganisationUnitFolder_Access(null);
				if ($folder_is_organisation_unit_folder_access->create($this->ci_organisation_unit_id, $folder_id) == null)
				{
					return false;
				}
